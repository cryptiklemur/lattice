...
export function ContextUtilizationChart({ data }: ContextUtilizationChartProps) {
  ...
  const { sessions, merged } = useMemo(function () {
    const sessionMap = new Map<string, { title: string; points: Array<{ messageIndex: number; contextPercent: number }> }>();
    for (let i = 0; i < data.length; i++) {
      const d = data[i];
      let entry = sessionMap.get(d.sessionId);
      if (!entry) {
    ...
    for (let si = 0; si < sessions.length; si++) {
      const pts = sessions[si][1].points;
    ...
    for (let mi = 0; mi <= maxIndex; mi++) {
      const row: Record<string, number> = { messageIndex: mi };
      for (let sj = 0; sj < sessions.length; sj++) {
        const point = sessions[sj][1].points.find(function (p) { return p.messageIndex === mi; });
