...
export function useTheme() {
  ...
  const allThemes = state.customThemes.length > 0
    ? themes.concat(state.customThemes)
    : themes;

  const currentThemeId = state.mode === "dark" ? state.darkThemeId : state.lightThemeId;
  const currentEntry = allThemes.find(function (e) { return e.id === currentThemeId; }) ?? themes[0];
