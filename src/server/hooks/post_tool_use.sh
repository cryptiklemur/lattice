#!/bin/sh
# Lattice Context Analyzer - PostToolUse hook for Claude Code
# Reads tool use JSON from stdin, forwards raw JSON to Lattice server
set -e
...
# Post raw JSON -- server handles parsing
curl -s -X POST "http://127.0.0.1:$PORT/api/hook/tool_use" \
